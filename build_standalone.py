...
license_files=[root/'LICENSE',*sorted((root/'licenses').glob('*'))]
...
parser=argparse.ArgumentParser();parser.add_argument('--output',type=Path,default=root/'Mooncake-Studio.html');args=parser.parse_args();p=args.output;p.parent.mkdir(parents=True,exist_ok=True);p.write_text(output);print(f'{p}: {p.stat().st_size:,} bytes')
